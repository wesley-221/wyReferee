...
export class ResizeDirective implements OnDestroy {
	...
	@Output() sizeChangedEnd = new EventEmitter<number>();
	...
	startResize(event: PointerEvent): void {
		...
		this.zone.runOutsideAngular(() => {
			this.removeMove = this.renderer.listen(this.document, 'pointermove', this.onResizeFn);
			this.removeUp = this.renderer.listen(this.document, 'pointerup', this.stopResizeFn);
		});
	}

	private onResizeFn = (event: PointerEvent): void => {
		if (!this.resizing) {
			return;
		}
	...
	private stopResizeFn = (): void => {
		...
		if (this.currentValue !== null) {
			this.zone.run(() => {
				this.sizeChangedEnd.emit(this.currentValue);
			});
